'use strict';
/*
 * store:EC-S3 shares px.x 1250 with gateway — the same detected centre —
 * but is ranked into column 3 while gateway stays in column 2.
 *
 * It is written in the file's own node order deliberately. store:EC-S3 comes
 * after gateway, so a stable sort by px presents 2 then 3, the adjacent-pair
 * walk sees a non-decreasing step, and review cycle 2's rank check said "ok".
 * Swapping the two entries — the same layout — rejected it. Equal centres
 * must get equal ranks whichever way round they are written.
 */
module.exports = {
