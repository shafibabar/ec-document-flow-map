'use strict';
/*
 * manual-run carries a px although it is drawn nowhere on the image.
 * 
 * The mirror of a rule cycle 1 added one field over — an inferred node must not
 * cite file/heading — left open, so an inferred node could carry a measurement of
 * a box that does not exist. The px chosen here is consistent with the node's own
 * rank, so the rank check stays silent and this fixture isolates one fault.
 *
 * Generated from data/layout.js by loading it, mutating and re-emitting.
 */
module.exports = {
